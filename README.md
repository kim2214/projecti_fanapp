...
- **오늘 방송했어요**: 오늘 방송을 마친 멤버를 모아 보여주며, 탭하면 치지직 채널 다시보기 목록으로 이동해 놓친 방송을 바로 볼 수 있습니다.
...
- 멤버의 방송이 시작·종료되면 서버가 보내는 무음 푸시로 즉시 갱신되고(앱 실행 시에도 갱신), 그 외에는 30분 주기로 서버 집계를 다시 읽습니다 (마지막 갱신 시각을 "HH:mm 기준"으로 표시).
...
- 최근 방송 기록(제목·길이·최고 시청자)을 보여주며, 탭하면 치지직 다시보기 목록으로 이동합니다.
- 최근 10회 방송의 **최고 동시 시청자 추이**를 미니 차트로 보여줍니다 (최고점·평균 표시).
- 치지직 **팔로워 수와 최근 30일 추이**를 보여줍니다 (서버가 공식 Open API로 매일 기록).
...
| 팔로워 수         | 치지직 공식 Open API (`/open/v1/channels`, Client 인증, 서버 일 1회) |
